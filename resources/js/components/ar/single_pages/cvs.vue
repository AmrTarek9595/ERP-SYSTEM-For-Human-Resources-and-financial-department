<template>
    <img src="/img/loading.gif" style="position: absolute;left: 40%; top:30%; z-index: 1;" v-if="!user_type" >
        <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg" style="direction: rtl;" v-if="hr">
    ...
    {{faildorsuccessmessage}}
        ...
        <li class="breadcrumb-item text-sm text-dark active" aria-current="page">الموارد البشرية</li>
        </ol>
        <h6 class="font-weight-bolder mb-0">السيرة الذاتية</h6>
        </nav>
...
<path d="M491.002,130.32c-9.715-5.609-21.033-7.099-31.871-4.196c-10.836,2.904-19.894,9.854-25.502,19.569L307.787,363.656
    ...
    <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">تاريخ الرفع</th>
    <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">المسمي الوظيفي</th>
    <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Iتاريخ المقابلة الشخصية</th>
    <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">الحالة الحالية</th>
    
    <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">تغيير الحالة</th>
...
<span class="mb-0 text-sm">{{ p.status }}</span>
    ...
    <select class="formbold-form-input"  v-model="canged_staus[p.id]" required>
      ...
      <option :value="'رفض - المرتب غير متوافق'">رفض - المرتب غير متوافق</option>
      <option value="رفض - الخبرة قليلة">رفض - الخبرة قليلة </option>
      <option value="رفض - الخبرة عالية">رفض - الخبرة عالية </option>
    ...
    <td class="align-middle text-left text-sm">
      <a :href="'/cv/'+p.cv_file">
    <span class="mb-0 text-sm">الملف </span>
      </a>  
  </td>

    <td class="align-middle text-left text-sm">
    <span class="mb-0 text-sm"><button class="btn btn-primary" type="submit" v-on:click="dochanged(p.id)">تغيير</button> </span>
    </td>
    </tr>
    ...
    <form action="" method="POST" enctype= multipart/form-data>
      ...
      <div class="mb-6 pt-4">
        <label class="formbold-form-label formbold-form-label-2">
          الملف: (Accept files with .pdf only )
        </label>
